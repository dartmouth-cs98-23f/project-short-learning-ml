...
def load_datapoint(id: str, second: int, transcript_df: DataFrame) -> Optional[dict]:
  """
  ...
  frame_path = f"data/frames/{id}/{second}.jpg"
  ...
  """
    Transcript stuff, ignore for now.
  """
  transcript_values = transcript_df[transcript_df['start'].between(second - 5, second + 5)]
...
def load_data(id: str) -> DataFrame:
  ...
  if not os.path.exists(f"data/frames/{id}") or not os.path.exists(f"data/transcripts/{id}.csv"):
    process_video(id)
  ...
  datapoints = [ load_datapoint(id, i, transcript_df) for i in range(len(frames)) ]
...
def load_transcript(id: str):
  if not os.path.exists(f"data/transcripts/{id}.csv"):
    process_video(id)
  return pd.read_csv(f"data/transcripts/{id}.csv", sep=',', encoding="utf-8")
